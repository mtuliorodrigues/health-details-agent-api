...
router.get('/collectors/vendors', (req, res) => res.json({ status: 'success', vendors: VENDORS }));
...
router.post('/devices/collect', async (req, res, next) => {
  ...
  if (!VENDORS[vendor]) return res.status(400).json({ status: 'error', message: 'Tipo de dispositivo inválido.' });
  try {
...
router.get('/devices/collect/stream', async (req, res) => {
  ...
  if (!validateIp(ip) || !isPrivateIp(ip) || !VENDORS[vendor]) return res.status(400).json({ status: 'error', message: 'IP ou tipo de dispositivo inválido.' });
