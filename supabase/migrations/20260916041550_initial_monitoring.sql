...
create policy "users read their devices" on public.devices
  for select to authenticated using ((select auth.uid()) = owner_id);

create policy "users read their client samples" on public.client_samples
  for select to authenticated using (
    exists (select 1 from public.devices where devices.id = client_samples.device_id and devices.owner_id = (select auth.uid()))
  );
